var SkyRTC = function() {
    ...
    skyrtc.prototype.connect = function(server, room, name) {
        ...
        that.room = room = room || "";
        that.name = name = name || that.getRandomString();
    ...
    skyrtc.prototype.sendMessage = function(message, socketId) {
        var that = this;
        if (that.dataChannels[socketId].readyState.toLowerCase() === 'open') {
    ...
    skyrtc.prototype.addDataChannel = function(socketId, channel) {
        ...
        channel.onmessage = function(message) {
            ...
            if (json.type === '__file') {
                ...
                that.emit('data_channel_message', channel, socketId, json.data.content, json.data.from);
            }
        };
