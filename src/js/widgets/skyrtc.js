var SkyRTC = function() {
    ...
    function skyrtc() {
        ...
        //保存所有与本地连接的socket的id
        this.connections = [];
        //保存uid与socket id的映射关系
        this.mappingU2S = {};
        //保存socket id与uid的映射关系
        this.mappingS2U = {};
        //初始时需要构建链接的数目
        this.numStreams = 0;
        //初始时已经连接的数目
    ...
    skyrtc.prototype.connect = function(server, room, uid) {
        ...
        this.on('_peers', function(data) {
            ...
            for(var i = data.connections.length;i--;) {
                that.mappingU2S[data.uids[i]] = data.connections[i];
                that.mappingS2U[data.connections[i]] = data.uids[i];
            }
            that.me = data.you;
        ...
        this.on('_new_peer', function(data) {
            ...
            that.mappingU2S[data.uid] = data.socketId;
            that.mappingS2U[data.socketId] = data.uid;
        ...
        this.on('_remove_peer', function(data) {
            ...
            delete that.mappingU2S[data.uid]
            delete that.mappingS2U[data.socketId];
    ...
    //获取某个uid的socketid
    skyrtc.prototype.getSocketId = function(uid){
        return this.mappingU2S[uid];
    };

    //获取某个socketid的uid
    skyrtc.prototype.getUid = function(sid){
        return this.mappingS2U[sid];
    };
